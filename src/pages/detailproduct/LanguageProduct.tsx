...
export const LanguageProduct = ({ nexasites }: DetailProductProps) => {
  return (
    <>
      <div className="shadow-md p-6 rounded-2xl bg-white dark:bg-slate-800 hover:shadow-lg dark:hover:outline dark:hover:outline-slate-600 dark:hover:outline-1">
        <div className="grid gap-4">
          <div className="text-lg font-bold">Programming Language</div>
          <hr />
          <div className="w-full md:mt-0 text-left grid grid-row-1 gap-4">
            {nexasites.nameTechStack.map((tech, index) => {
              const imagePath =
                nexasites.imageTechStack?.[index] ||
                "/image/skills/default.svg";
              const techLink = nexasites.linkTechStack?.[index] || "#";
